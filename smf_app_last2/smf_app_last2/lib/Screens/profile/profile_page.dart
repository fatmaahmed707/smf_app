...
class _ProfilePageState extends State<ProfilePage> {
  ...
  static const _privacySettingsKey = 'profile_privacy_settings';
  User? _currentUser;
  ...
  Future<void> _loadPreferences() async {
    ...
    final privacyRaw = prefs.getString(_privacySettingsKey);
    if (!mounted) return;
    setState(() {
  ...
  void _openSettingsPanel(_ProfilePanel panel) {
    showModalBottomSheet<void>(
      ...
      builder: (context) {
        ...
        switch (panel) {
          case _ProfilePanel.security:
            return _PasswordChangeSheet(
              palette: palette,
              user: _currentUser ?? _fallbackUser,
          ...
          case _ProfilePanel.privacy:
            return _PrivacySettingsSheet(
              ...
              onChanged: (key, value) {
                ...
                _saveBoolMap(_privacySettingsKey, _privacySettings);
              },
            );
        }
      },
            ...
            child: Column(
              children: [
                ...
                OutlinedButton(
                  ...
                  child: Text(languageProvider.getText('editProfile')),
                ),
              ],
            ),
          ),
    ...
    return Column(
      children: [
        ...
        _SettingsOptionCard(
          ...
          title: languageProvider.getText('securityPassword'),
          subtitle: languageProvider.getText('securityPasswordDesc'),
          color: _ProfileColors.green,
          onTap: () => onOpenPanel(_ProfilePanel.security),
        ),
        const SizedBox(height: 14),
        _SettingsOptionCard(
          palette: palette,
        ...
        _SettingsOptionCard(
          ...
          title: languageProvider.getText('privacySettings'),
          subtitle: languageProvider.getText('privacySettingsDesc'),
          color: _ProfileColors.cyan,
          onTap: () => onOpenPanel(_ProfilePanel.privacy),
        ),
        const SizedBox(height: 14),
        _SettingsOptionCard(
          palette: palette,
...
enum _ProfilePanel { security, notifications, privacy }
...
class _PasswordChangeSheetState extends State<_PasswordChangeSheet> {
  ...
  Future<void> _save() async {
    ...
    if (user.id.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Sign in first to update your password.')),
      );
      return;
    }
    ...
    try {
      await widget.usersService.updateUser(
        id: user.id,
        username: _profileDisplayName(user),
        email: user.email,
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not update password right now.')),
      );
    } finally {
      ...
      child: Column(
        ...
        children: [
          _SheetHeader(
            ...
            subtitle: 'Control visibility and activity preferences.',
          ),
          const SizedBox(height: 14),
          _SettingSwitchTile(
            palette: widget.palette,
            ...
            subtitle: 'Show profile details to team administrators.',
            value: _settings['profileVisibility'] ?? true,
            onChanged: (value) => _setValue('profileVisibility', value),
          ),
          _SettingSwitchTile(
            palette: widget.palette,
            ...
            subtitle: 'Keep recent account activity visible.',
            value: _settings['activityHistory'] ?? true,
            onChanged: (value) => _setValue('activityHistory', value),
          ),
          _SettingSwitchTile(
            palette: widget.palette,
            ...
            subtitle: 'Help improve SMF with diagnostic data.',
            value: _settings['usageAnalytics'] ?? false,
            onChanged: (value) => _setValue('usageAnalytics', value),
          ),
        ],
      ),
    );
